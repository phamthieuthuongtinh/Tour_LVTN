...
@section('content')
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title">Chỉnh Sửa Tour Giảm Giá</h3>
        </div>
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <select name="type_id" id="type_id" class="form-control">
                        ...
                        @foreach($type as $t)
                            <option {{$t->id==$discount->type_id ?'selected':''}} value="{{ $t->id }}">{{ $t->type_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select name="cate" id="cate" class="form-control" disabled>
                        ...
                        <option   {{$discount->cate->category_parent==6 ?'selected':''}} value="6">Ngoài nước</option>
                        <option   {{$discount->cate->category_parent==5 ?'selected':''}} value="5">Trong nước</option>
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select name="cate_id" id="cate_id" class="form-control" disabled>
                        @foreach($category as $c)
                            <option {{$c->id==$discount->category_id ?'selected':''}} value="{{ $c->id }}">{{ $c->title }}</option>
                        @endforeach
                ...
                <div class="form-group">
                    ...
                    <select name="tour_id" id="tour_id" class="form-control" disabled>
                        @foreach($tour as $to)
                            <option {{$to->id==$discount->tour_id ?'selected':''}} value="{{ $to->id }}">{{ $to->title }}</option>
                        @endforeach
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="rate" class="form-control" id="rate" placeholder="Nhập mức giảm giá" min="1" max="100" value="{{$discount->rate}}">
                </div>

                <!-- Thời gian bắt đầu -->
                <div class="form-group">
                    <label for="start">Ngày bắt đầu</label>
                    <input type="date"value="{{$discount->start}}" name="start" class="form-control" id="start">
                </div>

                <!-- Thời gian kết thúc -->
                <div class="form-group">
                    <label for="end">Ngày kết thúc</label>
                    <input type="date" name="end" value="{{$discount->end}}" class="form-control" id="end">
                </div>
            </div>
